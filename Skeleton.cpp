...
// Square Gait :- CH_C_1_PI/{0.2,0.4,4.0...}, int g = int(x/{0.2,0.4,4.0..})%4; 
// Figure of 8 Gaits :-  CH_C_1_PI_2 /{0.2,0.4,4.0...},  int g = int(x / {0.2,0.4,4.0..}) % 8; 
// The constructor is always initialized with the argument CH_C_1_PI/0.4f, the argument for the function initialization 
// is shown above
...
class ChFunction_LeftMotor : public ChFunction {
    public:
    // ChFunction_LeftMotor() : m_speed(CH_C_1_PI/0.2f) {} // Square Gait
      ChFunction_LeftMotor() : m_speed(CH_C_1_PI / 0.4f) {} // Figure of 8 symmetric Gaits
      //ChFunction_LeftMotor() : m_speed(CH_C_1_PI_2/4.0f){} // Works for geometric regime
    ...
    virtual double Get_y(double x) const override { 
     ...
     //   int g = int(x / 4.2f) % 4;
         //int g = int(x / 0.2f) % 4; // Works for Inertial regime

       // int g = int(x / 4.0f) % 8; //  works as expected
        int g = int(x / 1.0f) % 8; //  works as expected
...
class ChFunction_RightMotor : public ChFunction {
    public:
     //ChFunction_RightMotor() : m_speed(CH_C_1_PI/0.2f) {}      // Works for inertial regime
    ChFunction_RightMotor() : m_speed(CH_C_1_PI / 0.4f) {}  // Figure of 8 symmetry gaits
     //ChFunction_RightMotor() : m_speed(CH_C_1_PI_2/4.0f){}    // Works for geometric regime 
    ...
    virtual double Get_y(double x) const override { 
     ...
     //   int g = int(x/0.2f) % 4;       // Works for inertial
        //int g = int(x / 0.2f) % 4;   // Works for geometric
...
class Skeleton{
    public:
        ...
        void Initialize(){
            ...
            //auto left_motor_velo_func = chrono_types::make_shared<ChFunction_LeftMotor>(CH_C_PI / 0.2f);  // Works for geometric
            //auto left_motor_velo_func = chrono_types::make_shared<ChFunction_LeftMotor>(CH_C_PI / 0.2f);  // Square Gait in the inertial regime
            auto left_motor_velo_func = chrono_types::make_shared<ChFunction_LeftMotor>(CH_C_PI_2 / 1.0f);
              //auto left_motor_velo_func = chrono_types::make_shared<ChFunction_LeftMotor>(CH_C_PI / 0.4);
            //auto left_motor_velo_func = chrono_types::make_shared<ChFunction_LeftMotor>(CH_C_PI / 4.0f);  // Works for inertial
            ...
            // auto right_motor_velo_func = chrono_types::make_shared<ChFunction_RightMotor>(CH_C_PI/0.2f);  // Works for inertial
             // auto right_motor_velo_func = chrono_types::make_shared<ChFunction_RightMotor>(CH_C_PI / 0.2f);  // Symmetric Gait
            auto right_motor_velo_func = chrono_types::make_shared<ChFunction_RightMotor>(CH_C_PI_2 / 1.0f);  // Works for geometric
